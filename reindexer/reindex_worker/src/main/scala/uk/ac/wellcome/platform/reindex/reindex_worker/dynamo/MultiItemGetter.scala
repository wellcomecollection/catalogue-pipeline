...
import com.amazonaws.services.dynamodbv2.document.{BatchGetItemOutcome, DynamoDB, Item, TableKeysAndAttributes}
...
class MultiItemGetter(
  ...
  def get[T](
    ...
    for {
      batchGetResult: BatchGetItemOutcome <- Future { documentApiClient.batchGetItem(batchGetRequest) }
