...
object ReindexJobConfigBuilder extends Logging {
  def buildReindexJobConfigMap(config: Config): Map[String, ReindexJobConfig[SNSConfig]] =
    buildReindexJobConfigMap(
      config.requireString("reindexer.jobConfig")
    )

  def buildReindexJobConfigMap(jsonString: String): Map[String, ReindexJobConfig[SNSConfig]] = {
    val configMap = fromJson[Map[String, ReindexJobConfig[SNSConfig]]](jsonString) match {
