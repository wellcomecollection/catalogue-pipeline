...
@click.option("--type", "document_type", type=click.Choice(["works", "images"]), default="works")
...
def main(reindex_date, document_type, test_doc_id):
    ...
    dest_topic_arn_prefix = f"arn:aws:sns:eu-west-1:760097843905:catalogue-{reindex_date}"
    ...
    for hit in tqdm(
        scan(es, scroll="15m", index=api_index, query={ "query": api_index_query }, _source=False), total=reingest_docs_count
    ):
        doc_id = hit["_id"]
        sns.publish(
            TopicArn=dest_topic_arn,
