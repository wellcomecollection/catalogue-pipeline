  ...
  it("reads a blob from GitHub"){
    withWiremock("localhost") { port =>
      withActorSystem { implicit ac =>
        val url = Uri(s"http://localhost:$port/git/blobs/2e6b5fa45462510d5549b6bcf2bbc8b53ae08aed")
        val gitHubBlobReader = new GitHubBlobReader()
        whenReady(gitHubBlobReader.getBlob(url)) { result =>
          result shouldBe a[Right[_, _]]
          result.right.get shouldBe IOUtils.resourceToString("/WMS_Arabic_1.xml", StandardCharsets.UTF_8)
        }
      }
    }
  }
