  ...
  it("an older message for a file changed is ignored") {
    withWorkerService() {
      case (QueuePair(queue, dlq), messageSender, store, bucket, repoUrl) =>
        ...
        val modifiedTime =
          ZonedDateTime.parse(createdTime).minus(2, ChronoUnit.HOURS)
        val message =
          s"""
          {
            "path": "Arabic/WMS_Arabic_1.xml",
        ...
        eventually {
          ...
          val changeMessage = TeiIdChangeMessage(
            id = "manuscript_15651",
            s3Location = expectedS3Location,
            ZonedDateTime.parse(createdTime))
          val newKeyKey =
            s"tei_files/manuscript_15651/${modifiedTime.toEpochSecond}.xml"
      ...
      case (QueuePair(queue, dlq), messageSender, store, bucket, repoUrl) =>
        ...
        eventually {
          ...
          val deletedMessage = TeiIdDeletedMessage(
            id = "manuscript_15651",
            ZonedDateTime.parse(deletedTime))
          messageSender
