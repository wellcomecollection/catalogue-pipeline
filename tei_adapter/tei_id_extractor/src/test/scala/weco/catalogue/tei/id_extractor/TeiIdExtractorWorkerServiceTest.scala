    ...
    withLocalSqsQueuePair() { case q@QueuePair(queue, dlq) =>
      withActorSystem { implicit ac =>
        ...
        withSQSStream(queue) { stream: SQSStream[NotificationMessage] =>
          ...
          val service = new TeiIdExtractorWorkerService(messageStream = stream, messageSender = messageSender, gitHubBlobReader= gitHubBlobReader,idExtractor = new IdExtractor, store = store, pathIdDao = new PathIdDao, config = TeiIdExtractorConfig(concurrentFiles = 10, bucket = bucket.name))
          service.run()
