...
class PathIdManager(pathIds: PathIdTable) {
  ...
  def handlePathChanged(path: String, id: String, timeModified: Instant): Future[Option[PathId]] = {
  ...
  def handlePathDeleted(path: String, timeDeleted: Instant): Future[Option[PathId]] = DB localTx { implicit session =>
    Future {
      ...
      maybePathId match {
        ...
        case None => throw new RuntimeException(s"No path found for $path!")
      }
    }
  }
