...
case object WorkQueryableValues {
  ...
  def apply(
    ...
    new WorkQueryableValues(
      ...
      contributorsAgentLabel = data.contributors.map(_.agent.label).map(queryableLabel),
      description = data.description,
      edition = data.edition,
      genresConceptsLabel = data.genres.flatMap(_.concepts).map(_.label).map(queryableLabel),
      id = canonicalId.underlying,
      sourceIdentifierValue = sourceIdentifier.value,
      identifiersValue = (sourceIdentifier +: data.otherIdentifiers).map(_.value),
      imagesId = data.imageData.map(_.id).canonicalIds,
      imagesIdentifiersValue = data.imageData.map(_.id).sourceIdentifiers.toList,
      itemsId = data.items.map(_.id).canonicalIds,
      itemsIdentifiersValue = data.items.flatMap(_.id.allSourceIdentifiers).map(_.value),
      ...
      productionLabel = data.production.flatMap(
        p => p.places.map(_.label) ++ p.agents.map(_.label) ++ p.dates.map(_.label)
      ),
      referenceNumber = data.referenceNumber.map(_.underlying),
      subjectsConceptsLabel = data.subjects.flatMap(_.concepts).map(_.label).map(queryableLabel),
      title = data.title
    )
