...
class CollectionPathRuleTest
  ...
  it("puts the calm collectionPath on the tei work") {
  ...
  it("doesn't copy the collectionPath from a non calm work") {
    inside(
      CollectionPathRule.merge(
        teiWork,
        List(sierraIdentifiedWork().collectionPath(expectedCollectionPath)))) {
      case FieldMergeResult(collectionPath, _) =>
