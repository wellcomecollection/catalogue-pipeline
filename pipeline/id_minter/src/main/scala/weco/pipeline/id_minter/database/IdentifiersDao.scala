...
class IdentifiersDao(identifiers: IdentifiersTable) extends Logging {
  ...
  def lookupIds(
    ...
    Try {
      ...
      info(
        s"Found identifiers (${existingIdentifiers.size}): ${existingIdentifiers}"
      )
  ...
  private def rowMatcherFromSyntaxProvider(
  ...
  private def withTimeWarning[R](
    threshold: Duration,
    identifiers: Seq[SourceIdentifier]
  )(f: => R): R = {
