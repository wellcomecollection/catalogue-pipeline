...
class IdMinterStepFunctionLambdaTest
    extends AnyFunSpec
    with Matchers
    with ScalaFutures
    with WorkGenerators {
  ...
  def createMockProcessor(
    successfulSourceIds: List[String],
    failedSourceIds: List[String] = List.empty
  ): MintingRequestProcessor = {
    new MintingRequestProcessor(null, null)(ExecutionContext.global) {
      override def process(
        sourceIdentifiers: Seq[String]
      ): Future[MintingResponse] = {
        val successes = sourceIdentifiers
          .filter(successfulSourceIds.contains)
          .map(
            _ => "minted-id"
          ) // Canonical IDs don't matter for Step Function response
        val failures = sourceIdentifiers.filter(
          id =>
            failedSourceIds.contains(id) || !successfulSourceIds.contains(id)
        )
  ...
  describe("IdMinterStepFunctionLambda") {
    describe("processRequest") {
      ...
      it("handles partial and complete minting failure scenarios") {
        ...
        scenarios.foreach {
          case (name, sourceIds, successfulIds) =>
            withClue(s"Scenario: $name => ") {
              ...
              result.failures
                ...
                .toSet shouldBe failedIds.toSet
      ...
      it(
        "returns empty response (no failures) when source identifiers list is empty and does not invoke processor"
      ) {
      ...
      it("handles invalid input validation") {
        ...
        result.failures.head.error should include(
