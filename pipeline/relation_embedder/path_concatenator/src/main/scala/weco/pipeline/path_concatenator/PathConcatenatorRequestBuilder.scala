...
class PathConcatenatorRequestBuilder(index: Index) {
  ...
  /** 10K is the largest response size ES normally allows from non-scroll queries. In reality, the
    * largest possible response to a childWorks query (given the current data) will be less than
    * 1400.
    ...
    *   - Because of the order of processing, it is unlikely, but possible for all of them to be
    *     returned at once.
    *
    * Making a request with a larger size limit than required should not cause any degradation in
    * performance, so setting it to the ES default index.max_result_window should be harmless and
    * will ensure plenty of space for the collection to grow.
    */
  val maxResponseSize: Int = 10000

  /** A query that returns the collectionPaths from any works whose path ends with the first part of
    * this path.
    *
    ...
    *
    * However, because there is a possibility that errors in the data might cause this query to
    * yield more than one result, this request leaves the size constraint set at the default (10),
    * in order to allow the caller to log any excess results returned to help diagnose the problem
    */
  ...
  /** A query that returns the Work that has the exact path given in `path` As with parentPath, the
    * query is expected to return one record, but the default size constraint allows more records to
    * be returned to help with debugging.
    */
  ...
  /** A query that returns all works whose collectionPath starts with the final part of this path.
    ...
    * and it is likely that there will be more than 10 to return in any given run through the
    * concatenator.
    */
