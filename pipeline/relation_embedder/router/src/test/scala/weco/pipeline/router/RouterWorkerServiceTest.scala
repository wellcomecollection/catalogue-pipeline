...
import weco.catalogue.internal_model.work.{Relation, Relations, Work}
...
class RouterWorkerServiceTest
  ...
  it("sends collectionPath to paths topic") {
    val work = mergedWork().collectionLabel(CollectionPath("a"))
  ...
  it("a work with relations and collection path is error"){
    val work = mergedWork(relations = Relations(children = List(Relation(work = mergedWork(), depth = 1, numChildren = 0, numDescendents = 0)))).collectionLabel(CollectionPath("a"))
    val indexer = new MemoryIndexer[Work[Denormalised]]()
  ...
  it("sends on an invisible work") {
    val work =
      mergedWork().collectionLabel(CollectionPath("a/2")).invisible()
