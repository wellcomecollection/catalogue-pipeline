...
class SelectorTest extends AnyFunSpec with Matchers {
  ...
  it("generates selectors for multiple paths filtering any unnecessary") {
    Selector.forPaths(List("A/C", "A/C/X", "A/C/Y", "A/E", "A/E/3")) should contain theSameElementsAs List(
      Node("A") -> 0,
      Children("A") -> 0,
      Descendents("A/C") -> 0,
      Descendents("A/E") -> 3
  ...
  it("generates selector for whole tree when root is in the path") {
    Selector.forPaths(List("A/E/1", "A/B", "A", "A/B/D")) should contain theSameElementsAs List(
      Tree("A") -> 2
    )
  }

  it("generates selector for multiple disjoint trees") {
    Selector.forPaths(List("A/E", "A/E/2", "Other/Tree")) should contain theSameElementsAs List(
      Node("A") -> 0,
      Children("A") -> 0,
      Descendents("A/E") -> 0,
      Node("Other") -> 2,
