...
class TeiSubjectsTest extends AnyFunSpec with TeiGenerators with Matchers{
  ...
  it("doesn't extract subjects with an empty label"){
    val result = TeiSubjects(teiXml(
      id = id,
      profileDesc = Some(profileDesc(keywords = List(keywords(subjects = List(subject(""))))))
    ))
  ...
  it("extracts a subject with an lcsh id"){
    val result = TeiSubjects(teiXml(
      id = id,
      profileDesc = Some(profileDesc(keywords = List(keywords(keywordsScheme = Some("#LCSH"), subjects = List(subject("Botany", reference = Some("sh12345")))))))
    ))
  ...
  it("extracts the correct identifier if the subject id is prepended with subject_sh"){
    val result = TeiSubjects(teiXml(
      id = id,
      profileDesc = Some(profileDesc(keywords = List(keywords(keywordsScheme = Some("#LCSH"), subjects = List(subject("Torah scrolls", reference = Some("subject_sh 85136100")))))))
    ))

    result shouldBe List(Subject(id = Identifiable(SourceIdentifier(IdentifierType.LCSubjects, "Subject","sh85136100" )), label = "Torah scrolls", concepts = List(Concept(label ="Torah scrolls"))))
  }

  it("extracts a MESH subject"){
    val result = TeiSubjects(teiXml(
      id = id,
      profileDesc = Some(profileDesc(keywords = List(keywords(keywordsScheme = Some("#MeSH"), subjects = List(subject("Botany", reference = Some("subject_D001901")))))))
    ))

    result shouldBe List(Subject(id = Identifiable(SourceIdentifier(IdentifierType.MESH, "Subject","D001901" )), label = "Botany", concepts = List(Concept(label ="Botany"))))
  }
