...
with EitherValues{
  ...
  it("extracts author from msItem"){
    val result = TeiAuthors(node = msItem(s"${id}_1", authors = List(author(label = "John Wick"))), isFihrist = false)
  ...
  it("fails extracting authors if the label is empty"){
    val result = TeiAuthors(node = msItem(s"${id}_1", authors = List(author(label = ""))),isFihrist = false)
  ...
  it("extracts ids for authors"){
    val result = TeiAuthors(node = msItem(s"${id}_1", authors = List(author(label = "Dominic Toretto", key = Some("12534")))), isFihrist = false)
    result shouldBe a[Right[_, _]]
  ...
  it("if the id exists but it's an empty string, it extracts the author but doesn't add an id"){
    val result = TeiAuthors(node = msItem(s"${id}_1", authors = List(author(label = "Frank Martin", key = Some("")))), isFihrist = false)
    result shouldBe a[Right[_, _]]
    result.value shouldBe List(Contributor(Person("Frank Martin"), List(ContributionRole("author"))))
  }
  it("extracts author from persName"){
    val result = TeiAuthors(node = msItem(s"${id}_1", authors = List(author(persNames = List(persName(label = "John Connor", key = Some("12345"))), key = None))), isFihrist = false)
