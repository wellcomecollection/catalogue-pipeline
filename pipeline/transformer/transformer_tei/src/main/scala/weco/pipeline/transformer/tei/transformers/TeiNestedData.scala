...
object TeiNestedData extends Logging {
  ...
  /** TEI works can be composed of other works. This function extracts the
    * information about these nested works.
    *
    * Nested works can be specified in TEI as msItem or msPart depending if the
    * manuscript is a single part manuscript or a multipart manuscript. check
  ...
  /** Extract information about inner works for multi part manuscripts. Multi
    * part manuscripts have msPart elements containing information about inner
    * works. msParts don't have a title so we construct the title concatenating
    * the title of the wrapper work and the part number.
    */
  ...
  /** Extract information about inner works for single part manuscripts. For
    * single part manuscripts, inner works are described in msItem elements.
    */
  ...
  /** Manuscripts in the Fihrist catalogue - the Arabic manuscripts - are
    * catalogued to a higher level of granularity and it's not necessarily true
    * in this case that a msItem is a work. They are difficult to update to make
    * them more similar to other manuscripts so, for now, we just don't extract
    * lower level items for manuscripts in the Fihrist catalogue.
    */
  ...
  /** In an XML like this: <TEI xmlns="http://www.tei-c.org/ns/1.0"
    * xml:id="manuscript_15651"> <teiHeader> <fileDesc> <publicationStmt> <idno
    * type="msID">Wellcome Malay 7</idno> <idno type="catalogue">The Hervey
    * Malay Collection in the Wellcome Institute</idno> <idno
    * type="catalogue">Catalogue of Malay manuscripts in the Wellcome Institute
    * for the History of Medicine</idno> </publicationStmt> Extracts the values
    * of idno tags with type "catalogue"
    */
  ...
  /** In an XML like this: <TEI xmlns="http://www.tei-c.org/ns/1.0"
    * xml:id="manuscript_15651"> <teiHeader> <fileDesc> <titleStmt>
    * <title>Wellcome Library</title> </titleStmt> <sourceDesc> <msDesc
    * xml:lang="en" xml:id="MS_Arabic_1"> <msContents> <msItem
    * xml:id="MS_Arabic_1-item1"> <title xml:lang="ar-Latn-x-lc"
    * key="work_3001">Al-Qānūn fī al-ṭibb</title> extract the title from the
    * msItem, so "Al-Qānūn fī al-ṭibb" in the example.
    */
