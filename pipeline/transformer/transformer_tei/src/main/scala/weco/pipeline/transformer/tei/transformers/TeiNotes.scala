...
object TeiNotes {
  ...
  def apply(node: NodeSeq): List[Note] =
    getLocus(node) ++ getColophon(node).toList ++ getIncipitAndExplicit(node) ++ getHandNotes(node)
   ...
   *         </msPart>
   */
  def getHandNotes(nodeSeq: NodeSeq): List[Note] = (nodeSeq \ "physDesc" \ "handDesc" \ "handNote" ).flatMap { n =>
