...
/** Parses one period label per input line the way a ǂy subdivision is parsed
  ...
  * line. Run it with sbt: sbt "transformer_common/Test/runMain
  * weco.pipeline.transformer.parse.PeriodParserCli in.txt out.jsonl"
  */
object PeriodParserCli extends LabelDerivedIdentifiers {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val lines =
      Files.readAllLines(Paths.get(inPath), StandardCharsets.UTF_8).asScala
    val out = lines.map {
      raw =>
        ...
        val rangeJson = range match {
          case Some(r) =>
            s"""{"from": ${quote(r.from.toString)}, "to": ${quote(
                r.to.toString
              )}}"""
          case None => "null"
        }
        s"""{"input": ${quote(raw)}, "label": ${quote(label)}, "id": ${quote(
