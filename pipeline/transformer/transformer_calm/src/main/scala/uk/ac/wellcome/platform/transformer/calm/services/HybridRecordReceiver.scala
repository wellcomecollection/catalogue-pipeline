...
abstract class HybridRecordReceiver[MsgDestination, MsgIn](
  ...
  protected def getTransformable(msg: MsgIn): Try[CalmSourceData]
}

class BackwardsCompatHybridRecordReceiver[MsgDestination](
  msgSender: BigMessageSender[MsgDestination, TransformedBaseWork],
  store: Store[ObjectLocation, TypedStoreEntry[CalmSourceData]])
  ...
  protected def getTransformable(record: HybridRecord): Try[CalmSourceData] =
    record match {
...
class UpcomingHybridRecordReceiver[MsgDestination](
  ...
  store: Store[Version[String, Int],
               HybridStoreEntry[CalmSourceData}, EmptyMetadata]])
  ...
  protected def getTransformable(msg: UpcomingMsg): Try[CalmSourceData] =
