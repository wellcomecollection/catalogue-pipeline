...
import uk.ac.wellcome.platform.transformer.calm.service.{
  CalmTransformerWorkerServiceState,
  MessageReadError
}
...
trait Worker[Store, StoreKey, SourceData, SinkData] extends Runnable {
  ...
  def runStream(stream: SQSStream[NotificationMessage]) =
    stream.foreach(this.getClass.getSimpleName, processMessage)

  def processMessage(message: NotificationMessage) =
    for {
  ...
  private def readMessage(message: NotificationMessage): Result[StoreKey] =
    fromJson[StoreKey](message.body).toEither match {
      case Left(err) =>
        Left(new Exception(s"Worker: Cannot read message, ${err}"))
      case _ => _
    }
