...
/** Convert links between identified Sierra entries into a hierarchical path.
  *
  * The Sierra linking fields 773 and 774, when specified with the identifier subfield "$w - Record
  * control number" represent a part-whole relationship akin to those found in CALM or TEI
  * resources.
  *
  ...
  *
  * At this point, the records are not 'Identified', moreover, the identifiers used in these two
  * fields are Worldcat ids (e.g. (Wcat)29062i), which do not get resolved by the id minter.
  *
  * Therefore, at this point we can only create the "link" via a common collection path, to be later
  * resolved by the relation embedder.
  *
  * A feature of these fields is that they often refer (via the $g - Related Parts subfield) to a
  * subsection of the whole. This subsection is not in itself an entity that exists and can be
  ...
  *   - 774 0 |gPage 5 :|tCharing Cross Hospital: a portrait of house surgeons. Photograph,
  *     1906.|w(Wcat)28914i They each refer to the other, but there is no "page 5" object. So,
  *     although page 5 might look like it should be a node in the collectionPath, it cannot be.
  *
  ...
  *   - the value must be turned into something the relation embedder expects (evidence points to
  *     this being underscores for spaces).
  *
  * The hierarchy of Sierra-based data is flatter than other systems. A node is normally only a host
  * or a constituent, but three-level hierarchies are possible. It is possible for record that is a
  * "host" to also be part of a series, but that relationship would not contain the $w subfield, so
  * is to be ignored here. Such series relationships are handled in SierraParents
  *
  * A constituent has exactly one host. It is possible to have multiple 773 entries, but, as with a
  * host being part of a series, entries without $w are handled in SierraParents and signify
  * membership of a Series, not a hierarchical collection.
  *
  * A host may have many constituents, but for the purpose of defining a CollectionPath, it only
  * matters that one exists, not how many there are.
  */
...
/** Return an optional CollectionPath for a bib with host entry fields.
  *
  * The host entry field (773) has been used both for the identified relationship with a reciprocal
  * 774 on the other record, and for unidentified Series membership.
  *
  * As such, a bib may have many host entry fields (773), but at most, only one of them is expected
  * to result in a CollectionPath. This will be the one with a $w tag
  *
  * It is possible, but unlikely, that a bib would have a mixture of both. The more common scenario
  * is that it would have either exactly one 773 field, and that field has a $w subtag, or it would
  * have multiple 773 fields, and none of them have a $w subtag.
  */
private object HostEntryFieldCollectionPath extends SierraQueryOps with Logging {
  ...
  /** Return the path from the parent to this record as a / separated string.
    ...
    *
    * The $g - related part value is included, if present, as part of the node corresponding to this
    * record, this allows for alphanumeric sorting to put everything in (roughly) the correct order
    *
    * $g is not included as a node of its own, because it refers to something that does not exist as
  ...
  /** Extract the Related Part value from a host entry field, if any. The Related Part ($g) subfield
    * contains a free-text name of the part of the host to which this record belongs. This may be
    * something like a page or volume number, with or without text like "vol." or "page: " etc. The
    * source data may contain punctuation, spaces etc, all of which are removed or replaced to make
    * a token similar to those the Relation Embedder receives from CALM and TEI
    */
