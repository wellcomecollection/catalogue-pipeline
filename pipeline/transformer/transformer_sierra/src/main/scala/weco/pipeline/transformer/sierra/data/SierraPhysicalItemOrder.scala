...
/** The order of physical items in Sierra is significant, and can affect
  ...
  *
  *     Thing you can request
  *     Please request top item
  *     Another thing you can request
  *     Please request top item
  *
  * This "Please request top item" occurs when two items have been bound together
  * (e.g. on the same frame) and they have to be requested together.
  *
  * This ordering is stored in a part of Sierra we can't access through the
  * REST API.  If you sort only by item number (which we used to do), you might
  * get something like:
  *
  *     Please request top item
  *     Please request top item
  ...
  *
  * Before we turned off wellcomelibrary.org, I scraped the "correct" ordering by
  * looking at the rendered HTML pages, and saved it in the JSON file that supplies
  * this class.  If there was a non-standard order on wl.org, we'll use the same
  * order on wc.org.
  *
  * Motivation:
  *
  ...
  *     noticeably worse.
  *   - Using the fixed ordering is simpler than writing heuristics (e.g. based on
  *     item label), which could make the ordering better or worse.
  *   - Although this won't stay up-to-date with changes in Sierra, it gets us a
  *     reasonable improvement for not much effort.
  *
  * Long-term, we should find a better approach to this, but this alone represents
  * a non-trivial improvement in how we handle items.
  *
