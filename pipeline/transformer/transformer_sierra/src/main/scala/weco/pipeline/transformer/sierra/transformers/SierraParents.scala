...
object SierraParents extends SierraQueryOps with Logging {
  ...
  /** Convert the parent link MARC fields into Relation objects.
    ...
    * (i.e. the content and subfields) in document order. As such, they may
    * contain punctuation (and spacing) intended to presented in that context
    * specifically. In this usage, we are separating the main content from the
    * subfield so this punctuation is not wanted.
    */
  ...
  /** Return the title of the parent object represented by the given VarField
    ...
    *
    * In practice, there should be no problem with looking for $t and $a on any
