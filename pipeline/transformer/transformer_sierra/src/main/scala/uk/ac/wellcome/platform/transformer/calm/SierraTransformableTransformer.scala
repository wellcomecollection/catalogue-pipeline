...
import uk.ac.wellcome.platform.transformer.calm.exceptions.{
...
import uk.ac.wellcome.platform.transformer.calm.transformers._
import uk.ac.wellcome.platform.transformer.calm.source.SierraMaterialType._
...
class SierraTransformableTransformer(sierraTransformable: SierraTransformable,
  ...
  def workDataFromBibData(bibId: SierraBibNumber, bibData: SierraBibData) =
    WorkData(
      ...
      collection = SierraCalmRefNo(bibId, bibData)
    )
