...
object SierraProduction extends SierraTransformer with MarcUtils {
  ...
  case class TransformInfo(primaryMarcField: Option[String], dateMarcField: Option[String]) extends Info {
    def info = (primaryMarcField, dateMarcField) match {
      ...
      case (Some(marcField), None) =>
        s"Production events extracted from field ${marcField}"
      case (Some(marcField), Some(dateField)) =>
        s"Production events extracted from field ${marcField} " +
          s"(dates not extractable, used ${dateField} for dates)"
    }
  }
