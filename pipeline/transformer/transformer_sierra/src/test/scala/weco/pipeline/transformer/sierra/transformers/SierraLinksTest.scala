...
class SierraLinksTest
  ...
  describe("Links between documents") {
    ...
    describe("773 - Host Item Entry fields") {
      forAll(
        Table(
          "tag",
          "t",
        ...
        (tag) =>
          it(
            s"returns a Series relation with the title from the ǂ$tag subfield"
          ) {
            val varFields = List(
              VarField(
                marcTag = "773",
      ...
      it(
        "extracts the title from the body of a 773 field, if title is absent"
      ) {
    ...
    it(
      s"trims known separators ( ${separators.mkString(" ")} ) between field and subfield"
    ) {
