...
class SierraContributorsTest
  ...
  describe("Harmonising contributor types") {
    info("""
      The ontolooy type of a contributor is estimated based on the MARC field 
      from which it has been extracted.  Sometimes, the same contributor is 
      present in multiple MARC fields, leading that estimate to yield different
      types for the same thing.
      
      Having found the same contributor with multiple types, the transformer 
      harmonises them so that all instances of the same thing in a document
      have the same type.
      """)
    it(
