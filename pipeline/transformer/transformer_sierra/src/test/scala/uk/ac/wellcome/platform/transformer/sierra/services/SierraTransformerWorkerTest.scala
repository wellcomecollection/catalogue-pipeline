...
class SierraTransformerWorkerTest
  ...
  override def createPayloadWith(id: String, version: Int)(
    ...
    : SierraSourcePayload = {
    val transformable = createSierraTransformableWith(sierraId = SierraBibNumber(id))
  ...
  override def setPayloadVersion(p: SierraSourcePayload, version: Int)(
    implicit store: MemoryTypedStore[S3ObjectLocation, SierraTransformable]): SierraSourcePayload = {
    val transformable: SierraTransformable = store.get(p.location).value.identifiedT
