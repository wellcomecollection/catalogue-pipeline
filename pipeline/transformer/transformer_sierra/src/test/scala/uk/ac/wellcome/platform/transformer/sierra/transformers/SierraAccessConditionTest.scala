...
class SierraAccessConditionTest
    extends AnyFunSpec
    with Matchers
    ...
    bibItemPairs
      ...
      .foreach {
        case (bibId, bibData, itemId, itemData) =>
          val ac = Try {
            SierraAccessCondition(bibId, bibData, itemId, itemData)
          }
    ...
    describe("and does not have any holds") {
      it("if it is available, then it has no access conditions") {
        ...
        val itemData = createSierraItemDataWith(
          ...
          location = Some(
            SierraSourceLocation(code = "wgmem", name = "Medical Collection"))
        )
      ...
      it("if it is at digitisation, then it is temporarily unavailable") {
        ...
        val itemData = createSierraItemDataWith(
          ...
          location =
            Some(SierraSourceLocation(code = "wgser", name = "Journals"))
        )
  ...
  describe("if an item is in the closed stores") {
    describe("and does not have any holds") {
      it("if it is available, then it can be requested online") {
        ...
        ac shouldBe List(
          AccessCondition(
            ...
            note = Some("Mount bears watermark: B M FABRIANO"))
        )
        status shouldBe ItemStatus.Available
      }
      ...
      it(
        "if the bib is by appointment, then the item cannot be requested online") {
        val bibData = createSierraBibDataWith(
          varFields = List(
            VarField(
              marcTag = Some("506"),
        ...
        val itemData = createSierraItemDataWith(
          ...
          fixedFields = Map(
            "79" -> FixedField(
              label = "LOCATION",
              value = "sepin",
              display = "Closed stores EPB Incunabula"),
            "88" -> FixedField(
              label = "STATUS",
              value = "y",
              display = "Permission required"),
        ...
        val itemData = createSierraItemDataWith(
          ...
          fixedFields = Map(
            "79" -> FixedField(
              label = "LOCATION",
              value = "sepbr",
              display = "Closed stores EPB Planchests"),
            "88" -> FixedField(
              label = "STATUS",
              value = "y",
              display = "Permission required"),
          ...
          location = Some(
            SierraSourceLocation(
              code = "sepbr",
              name = "Closed stores EPB Planchests"))
        )
      ...
      it("you need to ask at the enquiry desk") {
        ...
        val itemData = createSierraItemDataWith(
          ...
          location =
            Some(SierraSourceLocation(code = "sgser", name = "Offsite"))
        )
      ...
      it(
        "if the bib and item are both closed, then the item cannot be requested") {
        val bibData = createSierraBibDataWith(
          varFields = List(
            VarField(
              marcTag = Some("506"),
      ...
      it("and the item type does not permit requesting") {
        ...
        val itemData = createSierraItemDataWith(
          ...
          location = Some(
            SierraSourceLocation(
              code = "mfohc",
              name = "Closed stores Moving image and sound collections"))
        )
      ...
      it("if the item is on exhibition") {
        ...
        val itemData = createSierraItemDataWith(
          ...
          location =
            Some(SierraSourceLocation(code = "exres", name = "On Exhibition"))
        )
        ...
        val itemData = createSierraItemDataWith(
          ...
          varFields = List(
            VarField(
              fieldTag = Some("n"),
              content =
                Some("Temporarily unavailable: undergoing internal assessment")
            )
          )
        )
      ...
      it("if the item is temporarily unavailable for internal assessment") {
        ...
        val itemData = createSierraItemDataWith(
          ...
          location = Some(
            SierraSourceLocation(code = "hicon", name = "Closed stores Visual"))
        )
      ...
      it("if you're meant to request the top item") {
        ...
        val itemData = createSierraItemDataWith(
          ...
          location =
            Some(SierraSourceLocation(code = "bwith", name = "bound in above"))
        )
    ...
    describe("and has at least one hold") {
      ...
      it("if it's on the holdshelf, it can't be requested") {
        ...
        val itemData = createSierraItemDataWith(
          ...
          location = Some(
            SierraSourceLocation(code = "swms4", name = "Closed stores WMS 4"))
        )
