...
class SierraTransformerWorkerServiceTest
  ...
  def withWorkerService[R](store: VersionedStore[String, Int, SierraTransformable],
                           sender: BigMessageSender[String, TransformedBaseWork],
                           queue: Queue)(
    testWith: TestWith[SierraTransformerWorkerService[String],
                       R]): R =
  ...
  def withBrokenWorkerService[R](store: VersionedStore[String, Int, SierraTransformable],
                           sender: BigMessageSender[String, TransformedBaseWork],
                           queue: Queue)(
    testWith: TestWith[SierraTransformerWorkerService[String],
                       R]): R =
      withActorSystem { implicit actorSystem =>
        withSQSStream[NotificationMessage, R](queue) { sqsStream =>
          val workerService = new SierraTransformerWorkerService(
            ...
            override val transformer: Transformer[SierraTransformable] = (input: SierraTransformable, version: Int) => Left(new Exception("AAAAAArgh!"))
          }
          workerService.run()
          testWith(workerService)
        }
  ...
  private def brokenStore = {
    new MemoryVersionedStore[String, SierraTransformable](new MemoryStore(Map[Version[String, Int], SierraTransformable]()) with MemoryMaxima[String, SierraTransformable]) {

      override def put(id: Version[String, Int])(entry: SierraTransformable): WriteEither =
