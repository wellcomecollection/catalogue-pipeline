...
import weco.catalogue.internal_model.work.{InstantRange, Period, Place, ProductionEvent}
...
class MarcProductionEventParserTest extends AnyFunSpec with Matchers {
  ...
  it("parses marc strings to ProductionEvent") {
    MarcProductionEventParser("790922s1757    enk||||      o00||||eng ccam   ") shouldBe
      Some(
        ProductionEvent(
          label = "1757",
          agents = Nil,
  ...
  it("parses marc strings to ProductionEvent when empty created date") {
    MarcProductionEventParser("      s2003    enk050        0   vneng dngm a ") shouldBe
      Some(
        ProductionEvent(
          label = "2003",
          agents = Nil,
  ...
  it("parses marc strings to ProductionEvent when detailed date") {
    MarcProductionEventParser("030623e19081121ua                k0eng dnkm a ") shouldBe
      Some(
        ProductionEvent(
          label = "1908/11/21",
          agents = Nil,
  ...
  it("parses marc strings to ProductionEvent when place not given") {
    MarcProductionEventParser("030818q16001699                  kn    dnka a ") shouldBe
      Some(
        ProductionEvent(
          label = "1600-1699",
          agents = Nil,
  ...
  it("doesnt parse marc strings to ProductionEvent when unknown date") {
    MarcProductionEventParser("090914uuuuuuuuuxx                  engddnteuua") shouldBe
      None
  }
}
