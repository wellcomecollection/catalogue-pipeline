...
object MarcParents extends MarcDataTransformer with MarcFieldOps with Logging {
  ...
  /** Convert the parent link MARC fields into Relation objects.
    ...
    *   - Are unique
    *
    * MARC fields are designed to be output by concatenating the subfields in
    * document order. As such, they may contain punctuation (and spacing)
    * intended to presented in that context specifically. In this usage, we are
    * separating the main content from the subfield so this punctuation is not
    * wanted.
    */
  def apply(record: MarcRecord): List[Relation] = {
    record
      .fieldsWithTags("440", "490", "773", "830")
  ...
  private def titleFromVarField(field: MarcField): Option[String] =
    subFieldTags(field.marcTag).flatMap {
      tag =>
