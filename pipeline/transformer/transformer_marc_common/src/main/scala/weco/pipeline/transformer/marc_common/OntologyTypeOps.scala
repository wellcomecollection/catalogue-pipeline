...
object OntologyTypeOps extends Logging {
  ...
  sealed trait EntityWithOntologyType[Output] {
    ...
    /** Return the principal source identifier associated with the entity in
      * question.
      */
    def mainSourceIdentifier: Option[SourceIdentifier]

    /** Return a copy of the entity in question, with its ontologyType corrected
      * to newType
      */
    def copyWithNewType(newType: String): Output

    /** Return the sourceIdentifier of the wrapped object with its ontologytype
      * blanked.
      */
  ...
  case class SubjectWithOntologyType(original: Subject[IdState.Unminted])
    ...
    /** Return the subject's sourceIdentifer Subjects have their own
      * SourceIdentifier.
      */
    def mainSourceIdentifier: Option[SourceIdentifier] =
    ...
    /** Return a copy of the Subject, with its ontologyType changed to newType.
      ...
      * The subject's own sourceIdentifier, and, only in the case simple
      * subjects the sole Concept in the Subject's concepts list
      *
      * In a compound subject, the Concepts in the concepts list will all be
      * label-derived and do not have an authoritative identifier to match
      * against, so cannot be unified.
      */
  ...
  case class ContributorWithOntologyType(
    ...
    /** Return the contributor's Agent's sourceIdentifier. Contributors do not
      * have a sourceIdentifier of their own, so the identifier is that of
      * whatever resides within its agent property (Contributor _does_ extend
      * HasId, but is always unidentifiable)
      */
    def mainSourceIdentifier: Option[SourceIdentifier] =
      original.agent.id.allSourceIdentifiers.headOption

    /** Return a copy of this Contributor with the Agent having been assigned
      * the correct type.
      */
  ...
  /** Given a list of Abstract Concept Ontology Types, return the most specific
    * one from the list
    *
    * Given that there isn't a linear hierarchy of all types, the actual
    * algorithm for this is fairly crude. There are two "base" or "vague" types,
    * and all the others are equally as "specific". In real data, this function
    * will not be called for many documents, and when it is, it will almost
    * always be to resolve disharmony between one of the vague types and one
    * more specific one.
    */
  ...
  /** Generate a map of untyped (ontology-wise) sourceIdentifiers to the most
    * appropriate ontology type available.
    *
    * In any given document, there may be multiple objects that have the same
    * real-world referent. These are specified with the same authoritative
    * identifier, but because the ontologyTypes have been derived from the field
    * in which it was found, they will have different SourceIdentifiers.
    *
    * This map provides a way to correct these SourceIdentifiers (or at least
    * unify them to the type that seems most likely to be correct)
    */
      ...
      .flatMap {
        ...
        case (identifier, values) =>
          val bestType = mostSpecificType(
            values.map(value => value.mainSourceIdentifier.get.ontologyType)
          )
