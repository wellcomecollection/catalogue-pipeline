...
module "path_concatenator_temp_output_topic" {
  source = "../topic"

  name       = "${local.namespace}_path_concatenator_temp_output_topic"
  role_names = [module.path_concatenator.task_role_name]
}

module "path_concatenator_temp" {
  source = "../fargate_service"

  name            = "path_concatenator_temp"
  container_image = local.path_concatenator_image

  topic_arns = [
    module.router_temp_candidate_incomplete_paths_output_topic.arn,
  ]

  env_vars = {
    queue_parallelism = 10

    downstream_topic_arn = module.path_concatenator_temp_output_topic.arn
