...
locals {
  ...
  es_host_secret_name     = "elasticsearch/pipeline_storage_${var.pipeline_date}/private_host"
  es_port_secret_name     = "elasticsearch/pipeline_storage_${var.pipeline_date}/port"
  es_protocol_secret_name = "elasticsearch/pipeline_storage_${var.pipeline_date}/protocol"

  pipeline_storage_es_service_secrets = {
    for service in keys(local.service_index_permissions) : service => {
      es_host     = local.es_host_secret_name
      es_port     = local.es_port_secret_name
      es_protocol = local.es_protocol_secret_name
...
resource "elasticstack_elasticsearch_ingest_pipeline" "set_indexed_at" {
  name        = "set-indexed-at-${var.pipeline_date}"
