...
resource "aws_iam_role_policy" "allow_ingestor_loader_task_token_policy" {
  role   = module.ingestor_loader_ecs_task.task_role_name
  policy = data.aws_iam_policy_document.allow_ingestor_loader_task_token.json
}

resource "aws_iam_role_policy" "ingestor_loader_read_secrets_policy" {
  role   = module.ingestor_loader_ecs_task.task_role_name
  policy = data.aws_iam_policy_document.allow_catalogue_graph_secret_read.json
}

resource "aws_iam_role_policy" "ingestor_loader_read_pipeline_secrets_policy" {
  role   = module.ingestor_loader_ecs_task.task_role_name
  policy = data.aws_iam_policy_document.ingestor_allow_pipeline_storage_secret_read.json
}

resource "aws_iam_role_policy" "ingestor_loader_s3_read_policy" {
  role   = module.ingestor_loader_ecs_task.task_role_name
  policy = data.aws_iam_policy_document.ingestor_s3_read.json
}

resource "aws_iam_role_policy" "ingestor_loader_s3_write_policy" {
  role   = module.ingestor_loader_ecs_task.task_role_name
  policy = data.aws_iam_policy_document.ingestor_s3_write.json
}

resource "aws_iam_role_policy" "ingestor_loader_neptune_read_policy" {
  role   = module.ingestor_loader_ecs_task.task_role_name
