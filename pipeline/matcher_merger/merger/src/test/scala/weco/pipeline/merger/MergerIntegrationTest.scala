  ...
  Scenario("A single work with nothing linked to it") {
    withContext {
      implicit context =>
        ...
        val denormWork = context.getDenormalised(work)
        denormWork.data shouldBe work.data
//        denormWork.state should beSimilarTo(work.state)
        denormWork.state.mergedTime should beRecent()
    }
  }
...
//        context.getMerged(miro3) should beRedirectedTo(sierra)
        ...
        val mergedImages = context.getDenormalised(sierra).data.imageData
...
//        context.getDenormalised(mets) should beRedirectedTo(sierraPicture)
