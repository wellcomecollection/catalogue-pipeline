...
object ChildWork {
  ...
  /** Return the childWork, having prepended the collectionPath from parentWork.
    * The collectionPath of a work contains its own path from its parent to
    * itself, so if its parent is not root, it needs to replace the parent node
    * in its own path with the parent node's path.
    *
    ...
    * The last node of the parent path must match the first node of the child
    * path.
    *
