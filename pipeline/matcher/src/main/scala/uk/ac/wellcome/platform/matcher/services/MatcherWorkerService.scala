...
class MatcherWorkerService[DestinationConfig](
  messageStream: MessageStream[TransformedBaseWork],
  messageSender: MessageSender[DestinationConfig],
  ...
  def processMessage(work: TransformedBaseWork): Try[Unit] = {
    (for {
      identifiersList <- workMatcher.matchWork(work)
      _ <- messageSender.sendT(identifiersList)
    } yield ()).recover {
