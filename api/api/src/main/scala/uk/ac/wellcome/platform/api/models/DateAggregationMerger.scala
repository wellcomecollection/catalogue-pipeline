...
object DateAggregationMerger extends DateHelpers {
  ...
  /** Dynamically merges a set of date aggregation results based on the range
    ...
    */
  def apply(agg: Aggregation[Period[Id.Minted]],
            maxBuckets: Int = 20,
            range: DateBucketRange = Decade): Aggregation[Period[Id.Minted]] =
  ...
  private def mergeBuckets(
    buckets: List[AggregationBucket[Period[Id.Minted]]],
    yearRange: Int): List[AggregationBucket[Period[Id.Minted]]] =
    buckets
      ...
      .map {
        case (key, count) =>
          ...
          AggregationBucket(Period[Id.Minted](label, Some(range)), count)
      }

  private def yearFromPeriod(period: Period[Id.Minted]): Option[Int] =
