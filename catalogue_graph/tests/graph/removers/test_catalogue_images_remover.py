...
def _make_remover(entity_type: str = "nodes") -> CatalogueImagesGraphRemover:
...
def test_get_total_node_count() -> None:
    ...
    remover.neptune_client.get_total_node_count.return_value = 42

    assert remover.get_total_node_count() == 42
    remover.neptune_client.get_total_node_count.assert_called_once_with("Image")
...
def test_get_node_ids_to_remove() -> None:
    remover = _make_remover()
    remover.neptune_client.get_disconnected_node_ids.return_value = iter(
        ["img1", "img2", "img3"]
    )
    ...
    remover.neptune_client.get_disconnected_node_ids.assert_called_once_with(
