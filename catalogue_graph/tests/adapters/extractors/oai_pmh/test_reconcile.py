"""Tests for the OAI-PMH reconciliation audit step."""
...
from adapters.steps.oai_pmh import reconcile
...
def _runtime(
    *, server_count: int | None, harvested_count: int, source_ids: list[str] | None
    ...
    return ReconcileRuntime(config=config, store=store, adapter_name="axiell")
...
def test_handler_publishes_metric_and_returns_drift() -> None:
    ...
    with patch.object(OAIPMHReconcileReport, "publish") as mock_publish:
        response = reconcile.handler(runtime)
...
def test_handler_samples_missing_ids_on_drift_when_enumeration_supported() -> None:
    ...
    with patch.object(OAIPMHReconcileReport, "publish"):
        response = reconcile.handler(runtime, list_missing=True)
