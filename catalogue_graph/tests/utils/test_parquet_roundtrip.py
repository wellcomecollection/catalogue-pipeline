...
def test_schema_stability_and_round_trip() -> None:
    ...
    def _equivalent(a: pa.DataType, b: pa.DataType) -> bool:
        ...
        if (pa_types.is_string(a) or pa_types.is_large_string(a)) and (
            pa_types.is_string(b) or pa_types.is_large_string(b)
        ):
            return True

        # List vs large_list equivalence (including element widening)
        list_like_a = pa_types.is_list(a) or pa_types.is_large_list(a)
        list_like_b = pa_types.is_list(b) or pa_types.is_large_list(b)
        if list_like_a and list_like_b:
            # Extract element types
            elem_a = a.value_type  # type: ignore[attr-defined]
            elem_b = b.value_type  # type: ignore[attr-defined]
            return _equivalent(elem_a, elem_b)
        return False

    for expected_field, actual_field in zip(schema_first, read_back.schema):
        assert expected_field.name == actual_field.name
        if not _equivalent(expected_field.type, actual_field.type):
    ...
    assert pa_types.is_list(nested_field.type) or pa_types.is_large_list(
        nested_field.type
    )
