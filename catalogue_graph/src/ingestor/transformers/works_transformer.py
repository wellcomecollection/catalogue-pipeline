...
class ElasticsearchWorksTransformer(ElasticsearchBaseTransformer):
    ...
    def _transform_filter(self, extracted: ExtractedWork) -> WorkAggregatableValues:
        transformer = QueryWorkTransformer(extracted)
        work = extracted.work
        return WorkFilterableValues(
            format_id=work.data.format.id,
            work_type=work.data.work_type,
            production_dates_range_from=transformer.production_dates_from,
            ...
            subjects_concepts_id=[s.id.canonical_id for s in work.data.subjects],
            subjects_concepts_source_identifier=list(transformer.subject_identifiers),
            contributors_agent_label=[c.agent.label for c in work.data.contributors],
            contributors_agent_id=[c.agent.id.canonical_id for c in work.data.contributors],
            contributors_agent_source_identifier=list(transformer.contributor_identifiers),
            identifiers_value=list(transformer.identifiers),
            items_locations_license_id=list(transformer.license_ids),
            items_locations_access_conditions_status_id=list(transformer.access_condition_status_ids),
