from models.indexable_concept import CatalogueConceptRelatedTo

from .neptune_concept import (
...
class RawNeptuneRelatedConcepts:
    ...
    @property
    def fields_of_work(self) -> list[CatalogueConceptRelatedTo]:
        raw_related = self._get_related_concepts("field_of_work")
        return transform_related_concepts(raw_related)

    @property
    def people(self) -> list[CatalogueConceptRelatedTo]:
        raw_related = self._get_related_concepts("people")
        return transform_related_concepts(raw_related)

    @property
    def narrower_than(self) -> list[CatalogueConceptRelatedTo]:
        raw_related = self._get_related_concepts("narrower_than")
        return transform_related_concepts(raw_related)

    @property
    def broader_than(self) -> list[CatalogueConceptRelatedTo]:
        raw_related = self._get_related_concepts("broader_than")
        return transform_related_concepts(raw_related)

    @property
    def frequent_collaborators(self) -> list[CatalogueConceptRelatedTo]:
        raw_related = self._get_related_concepts("frequent_collaborators")
        return transform_related_concepts(raw_related)

    @property
    def related_topics(self) -> list[CatalogueConceptRelatedTo]:
        raw_related = self._get_related_concepts("related_topics")
        return transform_related_concepts(raw_related)

    @property
    def related_to(self) -> list[CatalogueConceptRelatedTo]:
        raw_related = self._get_related_concepts("related_to")
        return transform_related_concepts(raw_related)

    @property
    def referenced_together(self) -> list[CatalogueConceptRelatedTo]:
