...
```bash
# Trigger
uv run python -m adapters.oai_pmh.steps.trigger --adapter-type axiell --at 2025-11-17T12:15:00Z > /tmp/axiell_event.json

# Loader
uv run python -m adapters.oai_pmh.steps.loader --adapter-type axiell --event /tmp/axiell_event.json

# Reloader (fill gaps)
uv run python -m adapters.oai_pmh.steps.reloader --adapter-type axiell \
  --job-id gap-reload \
