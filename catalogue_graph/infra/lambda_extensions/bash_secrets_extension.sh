...
# Copied from https://github.com/wellcomecollection/platform-infrastructure/tree/main/images/dockerfiles/jvm_container_lambda
# This extension extracts secrets from environment variables and writes them to a config file that the main Lambda function can read from.
...
# Extract secret value from environment variable and write to .env file
extract_secret_value_to_config() {
  ...
  if [[ $env_var_value == "secret:"* ]]; then
    ...
    echo "$env_var_key=\"$secret_value\"" >> "$DOTENV_FILE"
  fi
}
