...
data "aws_iam_policy_document" "iceberg_write" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:s3tables:eu-west-1:760097843905:bucket/wellcomecollection-platform-axiell-adapter",
      "arn:aws:s3tables:eu-west-1:760097843905:bucket/wellcomecollection-platform-axiell-adapter/*"
    ]
  }
  ...
  statement {
    ...
    resources = [
      "arn:aws:s3tables:eu-west-1:760097843905:bucket/wellcomecollection-platform-axiell-adapter/table/*"
    ]
  }
}

# Policy for reading from the Axiell adapter S3 bucket
data "aws_iam_policy_document" "s3_read" {
  statement {
    ...
    resources = [
      "arn:aws:s3:::wellcomecollection-platform-axiell-adapter",
      "arn:aws:s3:::wellcomecollection-platform-axiell-adapter/*"
    ]
  }
}

# Policy for writing to the Axiell adapter S3 bucket
data "aws_iam_policy_document" "s3_write" {
  statement {
    ...
    resources = [
      "arn:aws:s3:::wellcomecollection-platform-axiell-adapter/prod/*",
    ]
  }
}
...
data "aws_iam_policy_document" "ssm_read" {
  statement {
    ...
    resources = [
      "arn:aws:ssm:eu-west-1:760097843905:parameter/catalogue_pipeline/axiell_adapter/*",
      "arn:aws:ssm:eu-west-1:760097843905:parameter/catalogue_pipeline/axiell_collections/*"
    ]
  }
...
resource "aws_iam_policy" "state_machine_lambda_policy" {
  ...
  description = "Allow state machine to invoke Axiell adapter Lambda functions"
...
resource "aws_iam_policy" "state_machine_eventbridge_put_policy" {
  name        = "axiell-adapter-state-machine-eventbridge-put-policy"
...
resource "aws_iam_policy" "state_machine_logging_policy" {
  name        = "axiell-adapter-state-machine-logging-policy"
...
resource "aws_iam_policy" "eventbridge_state_machine_policy" {
  name        = "axiell-adapter-eventbridge-state-machine-policy"
  description = "Allow EventBridge to start Axiell adapter state machine executions"
