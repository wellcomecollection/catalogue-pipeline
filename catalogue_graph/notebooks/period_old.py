"""The period parser as it was before sb/improve-period-parser, kept only so the validation
notebook can score it. `old_bounds` reproduces how production used it: production dates were
parsed directly, subject and genre subdivisions only when `should_create_range` let them through."""
...
def old_bounds(path: str, text: str) -> tuple[str, str] | None:
    """(from, to) as ISO strings, or None where the old parser gave no range."""
    if path != "production" and not should_create_range(text):
        return None
    try:
        rng = parse_period(text).range
    except ValueError:
        return None
    return (rng.from_time, rng.to_time) if rng else None
