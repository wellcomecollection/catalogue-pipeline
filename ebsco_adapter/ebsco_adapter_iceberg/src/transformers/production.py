...
def extract_production_from_fields(fields: Iterable[Field]):
    return [production for production in (single_production_event(field) for field in fields)
...
def single_production_event(field: Field) -> ProductionEvent | None:
    ...
    places = [SourceConcept(label=subfield, type="Place") for subfield in field.get_subfields("a")]
    agents = [SourceConcept(label=subfield, type="Agent") for subfield in field.get_subfields("b")]
    dates = [parse_period(subfield) for subfield in field.get_subfields("c")]
    function = None
    if field.tag == "260" and field.get_subfields("e", "f", "g"):
        places += [SourceConcept(label=subfield, type="Place") for subfield in field.get_subfields("e")]
        agents += [SourceConcept(label=subfield, type="Agent") for subfield in field.get_subfields("f")]
...
def parse_period(period: str) -> Period:
    ...
    r = DateTimeRange.model_validate({
        "label": period,
        "from": "",
        "to": ""
    })
