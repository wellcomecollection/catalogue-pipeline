...
from transformers.ebsco_to_weco import transform_record, create_deleted_work, EBSCO_IDENTIFIER_TYPE
...
def transform(
    ...
    works = [transform_record(record) for record in marc_records if valid_record(record)]

    if not works:
        errors.append({"stage": "transform", "id": work_id, "reason": "failed_transform"})
...
def _process_batch(
    ...
    for row in batch.to_pylist():
        ...
        if content:
            ...
            transformed.append(create_deleted_work(row))
