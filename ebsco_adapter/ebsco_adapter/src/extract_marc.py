...
def extract_marc_records(available_files, xml_s3_prefix, temp_dir, s3_store, batch_size=100):
    unpacked_files = {}
    for batch_name, file in available_files.items():
        batch_completion_flag = "completed.flag"
        batch_completion_flag_path = os.path.join(xml_s3_prefix, batch_name, batch_completion_flag)
        batch_completed = s3_store.file_exists(batch_completion_flag_path)

        if not batch_completed:
            if 'download_location' not in file:
                download_location = s3_store.download_file(file['upload_location'], temp_dir)
            else:
            ...
            batches = [dict(list(marc_records.items())[i:i + batch_size]) for i in range(0, len(marc_records), batch_size)]
            ...
            for i, batch in enumerate(batches):
                ...
                print(f"Uploaded {len(batch) - len(already_uploaded)} records, {len(already_uploaded)} already uploaded.")

            json_encoded_files_uploaded = json.dumps(files_uploaded)
            s3_store.create_file(batch_completion_flag_path, json_encoded_files_uploaded.encode("utf-8"))

            unpacked_files[batch_name] = files_uploaded
        else:
            print(f"Batch {batch_name} already unpacked.")
            downloaded_flag = s3_store.download_file(batch_completion_flag_path, temp_dir)
