...
def update_notifier(updates, notify_for_batch, s3_store, s3_bucket, xml_s3_prefix, sns_publisher):
    update_messages = []
    deleted_messages = []
    ...
    print(f"Sending {len(update_messages)} update messages and {len(deleted_messages)} delete messages.")
    ...
    notified_completion_flag_path = os.path.join(xml_s3_prefix, notify_for_batch, notified_completion_flag)
