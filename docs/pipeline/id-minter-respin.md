...
- **Refresh `infrastructure/critical`'s outputs before you re-plan the pipeline stack.** `output "id_minter_rds"` iterates over both id-minter modules, so a `-target=module.id_minter_rds_2026_07_03` apply leaves one of the output's dependencies out of scope and Terraform skips evaluating the output at all. The resources update, the output does not. The dated pipeline stack reads the master secret ARN out of that output through remote state, so it reports "No changes" while both lambdas still name the secret that was deleted with the old cluster, and nothing surfaces until the minter next runs. Refresh it with `./run_terraform.sh apply -refresh-only -target=module.id_minter_rds_2026_07_03 -target=module.id_minter_rds`, which cannot alter resources, then check `./run_terraform.sh output -json id_minter_rds` names the new secret before planning the pipeline stack.
