...
- **The destroy needs `skip_final_snapshot`.** Both RDS modules take it (since wellcomecollection/catalogue-pipeline#3515); it must be `true` on the dated cluster, which is a restored copy whose contents are not worth keeping. Without it the delete fails after deletion protection is already off.
...
- **The restore silently drops `enable_http_endpoint`.** A snapshot restore does not carry the Data API setting, nothing fails loudly (the lambdas use MySQL), and the dated pipeline stack plans clean because the cluster lives in `infrastructure/critical`. Re-plan `infrastructure/critical` after the cluster reports available and apply the one-line fix.
...
`SELECT COUNT(*)` times out through the Data API on a fresh restore (Aurora hydrates lazily from S3). Use `information_schema.TABLES.TABLE_ROWS` estimates, taken from both clusters so the comparison is like for like. A good restore looks like identical `canonical_ids` counts and an `identifiers` count a few hundred short of production, the growth since the snapshot time.
