...
object DisplayPartOf {
  ...
  /** The partOf hierarchy of DisplayRelations is constructed from a flat list of Relation objects
    ...
    *
    * There are effectively two distinct behaviours involved here (which is what makes this
    * necessary).
    *   1. There is a strict single-parent hierarchy defining an objects position within a
    *      Collection 2. An object can also be in a Series.
    *
    * A Series is a flat, unordered bag of Works, represented by a relation with no ID. A Work may
    * be in zero or more Series and in zero or one hierarchical position within a Collection.
    *
    * @param flatAncestors
    *   a list of all the ancestors of a relation with any hierarchy represented as a list ordered
    *   from furthest to nearest.
    * @return
    *   The result of reconstituting flatAncestors into any hierarchy that exists, alongside its
    *   Series Relations
    */
