  ...
  implicit def workIndexable[State <: WorkState]: Indexable[Work[State]] =
    new Indexable[Work[State]] {
      ...
      override def weight(work: Work[State]): Long =
        // As an estimate here we assume 10 relations (which each consist of a
        ...
        // equivilent to around 400 works without any relations.
        Math.round(
          1.0 + (work.state.relations.size / 10.0)
        )
    }
}
