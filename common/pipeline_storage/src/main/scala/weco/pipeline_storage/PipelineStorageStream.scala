...
/** Configuration for the processing of messages in batches.
  ...
  *
  * The intended use for this is where it can be significantly more efficient to process multiple
  * messages together, rather than one at a time. For example where downloading all the data to
  * service a bundle of messages incurs less overhead than making individual requests for the data
  * to service each message.
  *
  * ==Setting batchSize and flushInterval==
  *
  * The numbers for batchSize and flushInterval should be chosen based on how responsive you want
  * the service to be and how many messages the service can realistically handle in one go, as well
  * as the expected frequency with which messages are likely to be received.
  *
  * In normal running, where this activity is triggered by a few real humans manually modifying a
  * few records at a time, you might expect the flush interval to be reached most of the time.
  *
  ...
  *
  * @note
  *   The values for batchSize and flushInterval are related to the duration set externally for the
  *   expiry of the messages being consumed (visibility timeout in SQS).
  *
  * `((Message Expiry) - (Flush Interval))` must be long enough to allow `(Batch Size - 1)` messages
  * to be processed, otherwise those messages will expire and either be re-queued or fail
  * completely.
  *
  * @param batchSize
...
object PipelineStorageStream extends Logging {
  def batchIndexAndSendFlow[T, MsgDestination](
    ...
    Flow[(Message, List[T])]
      .collect {
        case (msg, items @ _ :: _) =>
          items.map(item => Bundle[T](message = msg, item = item, numberOfItems = items.size))
      }
                  ...
                  .map {
                    case (id, idx) =>
                      result(id) match {
                        case Left(err) =>
                          error(
                            s"Could not retrieve document with id: $id",
                            err
                          )
                          None
