//TODO: This suite's name suggests that it checks the configuration of
// fields in the indexConfig, but it specifically assesses the slashesAnalyzer
// and nothing else.
// I think there is still value in a suite of tests like this, but would better based on
// the config that actually gets written to Elasticsearch, and be more comprehensive.
// Alternatively, I could rewrite the setup for these tests to create an index in
// the new way
...
//      .map(
//        hit => decode[TestDoc](hit.sourceAsString).right.get
//      ) shouldBe results
//  }
//
...
//
//    it("text matches") {
//      withLocalElasticsearchIndex(config = testIndexConfig) {
//        index =>
