...
object IdState {
  ...
  /** Represents an ID that has been successfully minted, and thus has a canonicalId assigned.
    */
  ...
  /** Represents an ID that has not yet been minted, but will have a canonicalId assigned later in
    * the pipeline.
    *
    * @param identifiedType
    *   \- this is used in the ID minter. What type will this become after it gets a canonical ID?
    */
  ...
  /** Represents an ID that has no sourceIdentifier and thus impossible to have a canonicalId
    * assigned. Note that it is possible for this ID to be either pre or post minter.
    */
