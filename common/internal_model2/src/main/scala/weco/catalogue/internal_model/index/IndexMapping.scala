...
object IndexMapping {
  ...
  /** Return a JSON string suitable for posting to Elasticsearch as a Mapping
    * object.
    *
    * Given a JSON string representing the properties for an index mapping,
    * populate a "mappings" object accordingly.
    *
    * This essentially replaces the practice of calling methods on an elastic4s
    * MappingDefinition to update a mapping with values for dynamic and _meta
    * etc. (which do not work with a definition initialised with rawSource),
