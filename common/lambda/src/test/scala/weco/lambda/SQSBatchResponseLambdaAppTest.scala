...
import weco.lambda.helpers.{ConfigurationTestHelpers, SQSLambdaAppHelpers, SQSMessageHelpers}
...
class SQSBatchResponseLambdaAppTest
  ...
  it(
    ...
    val eventStrings = (1 to 10).map { _ =>
      randomAlphanumeric()
    }.toList

    val fakeEvents = createSQSEvents(eventStrings)

    val messagesToProcess = fakeEvents.getRecords.asScala.map { message =>
  ...
  it("returns a batch response with the retryable failures only, if there is a mixed result state") {
    // include retryable failures, permanent failures, and successful messages
    val eventStrings = (1 to 9).map { _ =>
      randomAlphanumeric()
    }.toList

    val fakeEvents = createSQSEvents(eventStrings)

    val messagesToProcess = fakeEvents.getRecords.asScala.toList.zipWithIndex.map { case (message, i) =>
      if (i % 3 == 0) {
        SQSLambdaMessageFailedRetryable(message.getMessageId, new Throwable("Failed, but retryable"))
      } else if (i % 2 == 0) {
        SQSLambdaMessageFailedPermanent(message.getMessageId, new Throwable("Failed, and permanent"))
      } else {
        SQSLambdaMessageProcessed(message.getMessageId)
      }
    }
