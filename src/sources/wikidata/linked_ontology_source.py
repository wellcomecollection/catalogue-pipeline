...
class WikidataLinkedOntologySource(BaseSource):
    ...
    def _stream_raw_edges(self) -> Generator[dict]:
        ...
        for raw_mapping in process_stream_in_parallel(
            ...
            # Only yield the mapping if the linked id corresponds to the selected `node_type`. For example, if we
            # want to stream Wikidata 'locations' edges but the referenced LoC id is a 'names' id, we skip it.
