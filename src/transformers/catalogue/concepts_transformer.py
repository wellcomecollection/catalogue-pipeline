...
class CatalogueConceptsTransformer(BaseTransformer):
    def __init__(self, url: str):
        ...
        self.id_label_checker = IdLabelChecker.from_source(node_type=["concepts", "locations"], source=["loc", "mesh"])
    ...
    def extract_edges(self, raw_node: dict) -> Generator[ConceptHasSourceConcept]:
        ...
        if not raw_concept.is_concept:
            ...
            if (raw_concept.source == "label-derived") and (raw_concept.type not in ["Person", "Organisation", "Agent"]):
                # Generate edges via label
                assert hasattr(self.id_label_checker, "inverse")
                for source_concept_id in self.id_label_checker.inverse.get(raw_concept.label.lower(), []):
                ...
                if (raw_concept.source != "nlm-mesh") or (self.id_label_checker.get(raw_concept.source_concept_id) == raw_concept.label.lower()):
                    yield ConceptHasSourceConcept(
                        from_id=raw_concept.wellcome_id,
