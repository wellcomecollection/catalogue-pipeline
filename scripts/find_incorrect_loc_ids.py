...
def find_dodgy_ids_in_json(file_as_json):
    ...
    return file_as_json["sierraId"], find_b_number(body_as_json), find_dodgy_ids_in_object(body_as_json)

def find_dodgy_ids_in_object(obj):
    return list(filter(is_not_loc_id, [id_from_varfield(varfield) for varfield in obj["varFields"] if should_be_loc_id(varfield)]))
...
def should_be_loc_id(varfield):
    return (
        varfield.get('marcTag') in ["648","650","651","655","100","110","700","710"]
        and varfield['ind2'] == '0'
    )
