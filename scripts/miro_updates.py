...
from generate_miro_image_suppressions_table import (cloned_repo, git, get_github_api_key)
    ...
    for page in paginator.paginate(TableName=TABLE_NAME):
        for item in page["Items"]:
            for ev in item.get('events', []):
                if ev['description'] == 'Change isClearedForCatalogueAPI from True to False':
                    yield {
                        'id': item['id'],
                        'message': ev['message'],
    ...
    with cloned_repo("git@github.com:wellcomecollection/private.git"):
        with open('miro-suppressions.md', 'w') as outfile:
            ...
            for (date, message), events in itertools.groupby(
                sorted(get_all_miro_suppression_events(), key=lambda e: e['date'], reverse=True),
        ...
        print(f'https://github.com/wellcomecollection/private/pull/{new_pr_number}')
