...
import weco.catalogue.sierra_linker.{LinkingRecord, SierraLinkerWorkerService, SierraLinkerWorkerServiceTestCases}

class HoldingsLinkerWorkerServiceTest
  extends SierraLinkerWorkerServiceTestCases[SierraHoldingsNumber, SierraHoldingsRecord]
  ...
  override def withWorkerService[R](queue: SQS.Queue, store: VersionedStore[SierraHoldingsNumber, Int, LinkingRecord], messageSender: MemoryMessageSender, metrics: MemoryMetrics = new MemoryMetrics)(testWith: TestWith[SierraLinkerWorkerService[SierraHoldingsNumber, SierraHoldingsRecord, String], R]): R =
