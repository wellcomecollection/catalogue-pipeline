...
class SierraItemMergerFeatureTest
  ...
  def putItem(itemVhs: SierraItemVHS,
              itemRecord: SierraItemRecord): itemVhs.VHSEntry =
    itemVhs
      .update(id = itemRecord.id.toString)(
  ...
  it("stores an item from SQS") {
    ...
    withLocalSqsQueue { queue =>
      withSierraWorkerService(queue, messageSender, itemStore, vhs) { service =>
        service.run()
