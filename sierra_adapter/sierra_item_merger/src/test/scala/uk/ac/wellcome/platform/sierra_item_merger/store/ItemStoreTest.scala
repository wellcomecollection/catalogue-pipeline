...
class ItemStoreTest extends AnyFunSpec with SierraAdapterHelpers    with SierraGenerators{
  ...
  it("returns an exception if the record exists with a lower version"){
    ...
    val itemRecordStore = new ItemStore(createStore[SierraItemRecord](Map(lowerVersionKey-> itemRecord)))
    val result = itemRecordStore.get(lowerVersionKey.copy(version=1))
    result shouldBe a[Left[_,_]]
    result.left.get shouldBe a[Throwable]
  }

  it("returns VersionExpectedConflictException if the record exists with a higher version"){
    val itemRecord = createSierraItemRecord
    val higherVersionKey = Version(itemRecord.id.withoutCheckDigit, 1)
    val itemRecordStore = new ItemStore(createStore[SierraItemRecord](Map(higherVersionKey-> itemRecord)))
