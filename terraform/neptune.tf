resource "aws_neptune_cluster" "experimental_graph_cluster" {
  cluster_identifier                   = "neptune-test"
  engine                               = "neptune"
...
resource "aws_secretsmanager_secret_version" "neptune_cluster_endpoint_value" {
  ...
  secret_string = aws_neptune_cluster.experimental_graph_cluster.endpoint
}
