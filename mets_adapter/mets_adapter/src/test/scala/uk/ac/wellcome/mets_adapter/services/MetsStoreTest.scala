...
class MetsStoreTest extends FunSpec with Matchers {
  ...
  it("should store new METS data") {
    ...
    store.storeMetsData("001", data) shouldBe Right(Some(data))
  ...
  it("should update METS data when newer version") {
    ...
    val store = new MetsStore(internalStore)
    store.storeMetsData("001", newData) shouldBe Right(Some(newData))
    internalStore.getLatest("001") shouldBe Right(
      Identified(Version("001", 1), newData)
    )
  }

  it("should not update METS data when older version") {
    ...
    val store = new MetsStore(internalStore)
    store.storeMetsData("001", newData) shouldBe Right(None)
    internalStore.getLatest("001") shouldBe Right(
      Identified(Version("001", 0), oldData)
    )
  }

  it("should not update METS data when file path unchanged") {
    val oldData = MetsData("old/file.xml", 2)
